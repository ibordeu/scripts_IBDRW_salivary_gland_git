...
t_max = [86]; % maximum simuation time (h)
% seed types: 
...
% Other parameters
v0 = 1; % um/h
noise_amplitude = pi/7; % \eta: noise aplitude for the active tips
...
pretruncMean = 100; pretruncSD = 20;
...
part_angle(1) = part_angle(1) + 2*(rand()-0.5)*dt*noise_amplitude;
part_angle(2) = part_angle(2) + (acos(1 - 2*rand())/2-1/2)*dt*noise_amplitude;
