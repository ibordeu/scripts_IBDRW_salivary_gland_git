...
plot_times = [2000,4000,9000]; % times at which to show density profiles.
...
for niter = 1:N_iters
    ...
    if i<=length(plot_times) && niter > plot_times(i) 
        i= i+1;
        plot(a((Ly+1)/2,:)./a_plus,'k-');
        plot(s((Ly+1)/2,:)./s_plus,'k-.');
        plot(i((Ly+1)/2,:)./i_plus,'k--');
        legend({'a: Active tips','s: Inactive tips','i: Ducts'},'Location','SouthWest'); legend boxoff
        pause(0.0001) 
    end
end
